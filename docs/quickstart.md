...
Two end-to-end examples — one for causal LM distillation, one for embedding distillation. Both run on CPU for prototyping and scale to GPU with a single config change.
...
- [Training reference →](training/index.md) — all config fields explained
- [DataPipeline →](data.md) — streaming, shuffle, custom columns
- [Skill Packs →](skillpacks.md) — attach LoRA adapters to your distilled student
